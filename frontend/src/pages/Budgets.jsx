...
const Budgets = () => {
  ...
  return (
    <DashboardLayout>
      ...
      {/* Header */}
      <PageHeader
        title="Monthly Budgets & Thresholds"
        subtitle="Track monthly limits, remaining funds, and automated 80%, 90%, and 100% threshold warnings."
        icon={Target}
        action={
          <button
      ...
      <div className="space-y-3">
        {criticalBudgets.length > 0 && (
          <div className="p-4 rounded-2xl bg-[#FEF2F2] border border-[#FECACA] text-[#DC2626] flex items-start space-x-3 text-xs">
            ...
            <div>
              <p className="font-semibold text-[#991B1B]">Critical Alert: Budget Exceeded (100%+)</p>
        ...
        {warningBudgets.length > 0 && (
          <div className="p-4 rounded-2xl bg-[#FFFBEB] border border-[#FDE68A] text-[#D97706] flex items-start space-x-3 text-xs">
            ...
            <div>
              <p className="font-semibold text-[#B45309]">High Usage Warning (90%+ Spent)</p>
              <p className="mt-0.5 text-[#D97706]">
        ...
        {cautionBudgets.length > 0 && (
          <div className="p-4 rounded-2xl bg-[#FFFBEB] border border-[#FDE68A] text-[#D97706] flex items-start space-x-3 text-xs">
            ...
            <div>
              <p className="font-semibold text-[#B45309]">Budget Caution (80%+ Spent)</p>
              <p className="mt-0.5 text-[#D97706]">
      ...
      {loading ? (
        <div className="p-12 flex flex-col items-center justify-center space-y-3 bg-white border border-[#E2E8F0] rounded-2xl">
          ...
          <p className="text-xs text-[#64748B]">Calculating monthly budget status...</p>
        </div>
      ) : error ? (
        ...
        <div className="p-12 text-center space-y-4 bg-white border border-[#E2E8F0] rounded-2xl shadow-xs">
          ...
          <div className="space-y-1">
            <p className="text-base font-semibold text-[#0F172A]">No Monthly Budgets Configured</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {budgets.map((b) => {
            ...
            let progressColor = 'bg-[#111827]';
            let badgeBg = 'bg-[#F1F5F9] text-[#475569] border-[#E2E8F0]';
            ...
            if (isExceeded) {
              ...
              statusText = isWarning ? '90%+ Warning' : '80%+ Caution';
            }

            return (
              <div
                key={b._id}
                className="p-6 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs space-y-5 relative overflow-hidden group hover:-translate-y-0.5 hover:shadow-md transition-all duration-200"
              >
                {/* Header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-xl bg-[#F1F5F9] border border-[#E2E8F0] text-[#111827] flex items-center justify-center font-bold text-xs">
                      <Target className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="text-base font-semibold text-[#0F172A] leading-tight">{b.category}</h3>
                      <p className="text-[10px] text-[#64748B] uppercase tracking-wider font-medium">Monthly Budget</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 p-3.5 bg-[#F8FAFC] rounded-xl border border-[#E2E8F0] text-xs">
                  <div>
                    ...
                    <span className="font-semibold text-[#0F172A] text-sm">{formatCurrency(b.spent)}</span>
                  </div>
                  <div>
                    <span className="text-[11px] text-[#64748B] block font-medium">Remaining</span>
                    <span className={`font-semibold text-sm ${isExceeded ? 'text-[#DC2626]' : 'text-[#16A34A]'}`}>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[#64748B] font-medium">Limit: {formatCurrency(b.amount)}</span>
